...
def locationProccess(location):
            ...
            if lsa_response.status_code ==200:
                ...
                url = "https://testsdsd.free.beeceptor.com"
                ...
                print(str(response.status_code) + " "+ str(name))
                time.sleep(0.5)
...
def Reviews_main(token,clientid):
...
#
    ...
    if lsa_response.status_code ==200:
        ...
        target=None
        client = f"locations/{clientid}"
        for location in locations:
            #print(location['name'])
            if location['name'] == client:
                print(location['name'])
                target=location
                locationProccess(target)
...
#
    ...
    if temptoken:
        print("NEW PAGE")
        Reviews_main(temptoken,clientid)
    return
def Execute_Main():
	Acc_info = Reviews_main(None,16493985034806803002)
